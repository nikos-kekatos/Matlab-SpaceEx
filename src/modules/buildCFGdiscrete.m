function  buildCFGdiscrete(options,sys)
...
var=sys.var;in=sys.in;
n=length(sys.A); k=size(sys.B,2);
output=options.output;
IC=options.IC;
% //To-do make it general
if n==4 && k==2
    fprintf(fileID,'initially = "t==0 & %s==%.4f & %s==%f & %s==%f & %s==%f & %s==0 & %s==0 & %s==0" \r\n',var{1},IC{1},var{2},IC{2},var{3},IC{3},var{4},IC{4},in{1},in{2},var{end});
...
fprintf(fileID,'output-format = %s \r\n',options.output_format);
